...
def test_validate_accepts_bidirectional_mapping():
    ranking = [RankingEntry("anime.yaml", "Example", "S", "ANIME", (1,))]
    collections = {"ANIME": {1: media_entry(1, 8)}, "MANGA": {}}

    assert validate(ranking, collections) == []


def test_validate_rejects_score_drift():
    ranking = [RankingEntry("anime.yaml", "Example", "S", "ANIME", (1,))]
    collections = {"ANIME": {1: media_entry(1, 7)}, "MANGA": {}}

    errors = validate(ranking, collections)

    assert any("expects score 8" in error for error in errors)


def test_validate_rejects_reverse_drift():
    collections = {"ANIME": {1: media_entry(1, 8)}, "MANGA": {}}

    errors = validate([], collections)

    assert any("no story-ranking mapping" in error for error in errors)


def test_validate_accepts_duplicate_id_for_multiple_ranking_entries():
    ranking = [
...
def test_validate_rejects_pending_and_ranked_duplicate():
    ranking = [
    ...
    assert any("both pending and ranked" in error for error in errors)


def test_load_allows_rows_without_anilist_ids():
...
def test_tierless_optional_file_can_cover_anilist_media():
...
def test_reverse_check_ignores_non_completed_anime():
    current = media_entry(1, 8)
    current["status"] = "CURRENT"

    assert validate([], {"ANIME": {1: current}, "MANGA": {}}) == []


def test_reverse_check_still_includes_current_manga():
